(function($){
	...
	$.extend($.fn.datagrid.defaults, {
		...
		filterPosition: 'top',
		remoteFilter: false,
		filterDefaultText:false,//add by @lvzc 2018-11-28 默认不显示过滤项  
		filterDelay: 400,
		filterRules: [],
	...
	function init(target, filters){
		...
		function createFilter(frozen){
			...
			for(var i=0; i<fields.length; i++){
				...
				if(!fopts && !opts.filterDefaultText){continue;}//add by @lvzc 2014-11-28 判断是否默认是否显示过滤项
				...
				if (fopts.op){//modify by @lvzc 2014-11-28 判断是否有定义规则类，未定义的使用默认过滤
