...
public class ItemNode {
	...
	public List<SpinnerDataInfo> spinnerDataInfos;

	@JsonProperty(value = "SpecSpinnerDataInfo")
	public List<SpecSpinnerDataInfo> specSpinnerDataInfos;
	...
	public Void addControlList(List list) {
		...
		List<SpinnerDataInfo> list6  = new ArrayList();
		List<Label> list7  = new ArrayList();
		String name;
		for (int i = 0; i< list.size();i++){
			...
			switch (name){
				...
				case "SpinnerDataInfo":
					list6.add((SpinnerDataInfo) list.get(i));
					childViewModelLists.add(new ChildViewModel(ChildViewModel.Spinner,list.get(i)));
					break;
				case "Label":
					list7.add((Label) list.get(i));
